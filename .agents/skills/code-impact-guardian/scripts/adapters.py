...
def detect_language_adapter(project_root: pathlib.Path, config: dict) -> str:
    ...
    if any(path.exists() for path in iter_matching_files(project_root, config.get("python", {}).get("source_globs", []) + config.get("python", {}).get("test_globs", []))):
        return PYTHON_ADAPTER
    if any(path.exists() for path in iter_matching_files(project_root, config.get("tsjs", {}).get("source_globs", []) + config.get("tsjs", {}).get("test_globs", []))):
        return TSJS_ADAPTER
...
def collect_adapter_graph(project_root: pathlib.Path, config: dict, adapter_name: str) -> AdapterGraph:
    if adapter_name == PYTHON_ADAPTER:
        return collect_python_graph(project_root, config)
    if adapter_name == TSJS_ADAPTER:
        return collect_tsjs_graph(project_root, config)
    return collect_generic_graph(project_root, config)
...
def collect_python_graph(project_root: pathlib.Path, config: dict) -> AdapterGraph:
    ...
    for path in iter_matching_files(project_root, patterns):
        ...
        file_records[relative] = {"path": relative, "is_test_file": is_test_file}
...
def collect_tsjs_graph(project_root: pathlib.Path, config: dict) -> AdapterGraph:
    adapter_graph = AdapterGraph()
    ...
    for path in iter_matching_files(project_root, patterns):
        ...
        adapter_graph.files.append({"path": relative, "is_test_file": is_test_file})
...
def collect_generic_graph(project_root: pathlib.Path, config: dict) -> AdapterGraph:
    adapter_graph = AdapterGraph()
    ...
    for path in matched_files:
        relative = path.relative_to(project_root).as_posix()
        adapter_graph.files.append({"path": relative, "is_test_file": False})
    return adapter_graph
