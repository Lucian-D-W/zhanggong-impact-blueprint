...
sys_path_root = pathlib.Path(__file__).resolve().parents[1] / ".agents" / "skills" / "code-impact-guardian" / "scripts"
import sys
...
def load_repo_config(repo_root: pathlib.Path) -> dict:
    return json.loads((repo_root / ".code-impact-guardian" / "config.json").read_text(encoding="utf-8"))


def load_cig_module(repo_root: pathlib.Path):
    module_path = repo_root / ".agents" / "skills" / "code-impact-guardian" / "cig.py"
...
class SelfHostingGuardFocusTest(unittest.TestCase):
    def test_repo_local_config_targets_guardian_repo_root(self):
        ...
        self.assertIn("scripts/**/*.py", config["python"]["source_globs"])
        self.assertIn(".agents/skills/code-impact-guardian/*.py", config["python"]["source_globs"])
        self.assertIn(".agents/skills/code-impact-guardian/**/*.py", config["python"]["source_globs"])
    ...
    def test_self_host_globs_ignore_nested_dist_copies(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            skill_source = repo_root / ".agents" / "skills" / "code-impact-guardian"
            skill_target = workspace_root / ".agents" / "skills" / "code-impact-guardian"
            ...
            dist_skill_target = workspace_root / "dist" / "code-impact-guardian-stage1-review-v2" / ".agents" / "skills" / "code-impact-guardian"
            dist_skill_target.parent.mkdir(parents=True, exist_ok=True)
            shutil.copytree(skill_target, dist_skill_target)
            dist_scripts = workspace_root / "dist" / "code-impact-guardian-stage1-review-v2" / "scripts"
            ...
            dist_tests = workspace_root / "dist" / "code-impact-guardian-stage1-review-v2" / "tests"
            ...
            self.assertIn(".agents/skills/code-impact-guardian/cig.py", source_matches)
            self.assertIn(".agents/skills/code-impact-guardian/scripts/parser_backends.py", source_matches)
            self.assertIn("scripts/demo_phase1.py", source_matches)
            self.assertNotIn("dist/code-impact-guardian-stage1-review-v2/.agents/skills/code-impact-guardian/cig.py", source_matches)
            self.assertNotIn(
                "dist/code-impact-guardian-stage1-review-v2/.agents/skills/code-impact-guardian/scripts/parser_backends.py",
                source_matches,
            )
            self.assertNotIn("dist/code-impact-guardian-stage1-review-v2/scripts/demo_phase1.py", source_matches)
            self.assertIn("tests/test_guard_focus.py", test_matches)
            self.assertNotIn("dist/code-impact-guardian-stage1-review-v2/tests/test_guard_focus.py", test_matches)
    ...
    def test_copy_template_omits_repo_local_config(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            (source_root / ".code-impact-guardian").mkdir(parents=True, exist_ok=True)
            (source_root / "scripts").mkdir(parents=True, exist_ok=True)
            (source_root / ".code-impact-guardian" / "config.json").write_text('{"project_root":"."}\n', encoding="utf-8")
            (source_root / "scripts" / "demo_phase1.py").write_text("print('demo')\n", encoding="utf-8")

            cig.copy_template(source_root, destination_root)

            self.assertFalse((destination_root / ".code-impact-guardian" / "config.json").exists())
    ...
    def test_in_place_demo_keeps_self_hosting_config(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            config = json.loads((temp_repo / ".code-impact-guardian" / "config.json").read_text(encoding="utf-8"))
            self.assertEqual(config["project_root"], ".")
